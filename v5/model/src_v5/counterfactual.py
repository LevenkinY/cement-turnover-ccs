...
def _reference_dispatch_preflight(
    ...
    *,
    ...
    # P0-3: the utilisation ramp constraint was removed; passing None disables the
    # ramp check, and the counterfactual dispatch validation then only enforces the
    # lower bound and y/u consistency.
    # P0-3: the utilisation ramp constraint was removed, so the turnover path
    # extraction no longer imposes a per-period utilisation cap.
    ramp = float("inf")
    ...
    for raw_plant_id, record in plants.items():
        ...
        for period_idx, year in enumerate(years):
            ...
            if period_idx > 0:
                ...
                # P0-3: passing maximum_utilization_change=None disables the ramp
                # check (the constraint was removed from the model).
                if maximum_utilization_change is not None:
                    up_gap = (
                        u_value
                        - prior_u
                        - maximum_utilization_change
