...
def _ccs_learning_config():
    try:
        from src_v5 import config_v5 as cfg

        scaling = getattr(cfg, "CCS_COST_SCALING_PARAMS", {})
        curve = getattr(cfg, "CCS_COST_DECLINE_SCENARIOS", {}).get(
            getattr(cfg, "CCS_COST_DECLINE_CASE", "central"), {}
        )
...
def residual_emissions_by_province(results, meta):
    ...
    if balance.empty:
        return balance
    # 2026-09-12 (U1): the province table used to report `utilization` as a SIMPLE
    # AVERAGE over ALL original lines, including retired lines at u=0 -- a different
    # denominator from the national capacity-weighted operating metric, yet sharing
    # the same name. Three distinct quantities are now reported explicitly:
    #   capacity_weighted_utilization  : sum(K*u) / sum(K*y)  -> PRIMARY metric
    #                                    (operating design capacity), same definition
    #                                    as the national summary
    #   output_over_baseyear_capacity  : sum(K*u) / sum(K)    -> production relative
    #                                    to the base-year fleet capacity; NOT a
    #                                    current industry utilization rate
    #   simple_average_utilization_all_lines : mean(u) over all original lines,
    #                                    kept only for continuity/provenance
    # `utilization` is kept as the PRIMARY metric so downstream consumers get the
    # correct definition under the established name.
    balance = balance.assign(
        _cap_u=balance["capacity"] * balance["utilization"],
        _cap_operating=balance["capacity"] * balance["operating"],
    )
    grouped = balance.groupby(["province", "period"], dropna=False).agg(
        capacity=("capacity", "sum"),
        operating_capacity=("_cap_operating", "sum"),
        production_proxy=("_cap_u", "sum"),
        operating_plants=("operating", "sum"),
        simple_average_utilization_all_lines=("utilization", "mean"),
    ...
    grouped["capacity_weighted_utilization"] = (
        grouped["production_proxy"] / grouped["operating_capacity"].replace(0, np.nan)
    )
    grouped["output_over_baseyear_capacity"] = (
        grouped["production_proxy"] / grouped["capacity"].replace(0, np.nan)
    )
    grouped["utilization"] = grouped["capacity_weighted_utilization"]
    grouped["utilization_definition"] = "operating_capacity_weighted"
