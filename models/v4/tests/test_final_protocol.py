...
class FinalProtocolTests(unittest.TestCase):
    ...
    def test_observed_base_year_production_and_af_are_anchored(self):
        ...
        fuel_ktce = clinker_kt * config.TCE_PER_T_CLINKER
    ...
    def test_storage_offshore_flags_exclude_land_false_positives(self):
        ...
        self.assertGreater(int(corrected.sum()), 0)
        self.assertTrue(
    ...
    def test_spatial_equalization_preserves_national_af_and_base_year(self):
        ...
        for year in config.T_LIST:
            before = sum(original["province_af_pool_ktce"].get((p, year), 0.0) for p in provinces)
            after = sum(equalized["province_af_pool_ktce"].get((p, year), 0.0) for p in provinces)
            self.assertAlmostEqual(before, after, places=6)
        for key, value in original["plant_af_access_ktce"].items():
            if int(key[1]) == 2025:
                self.assertAlmostEqual(equalized["plant_af_access_ktce"][key], value, places=9)

        plants = equalized["plants"].set_index("plant_id")
        for plant_id, row in plants.iterrows():
            annual_capacity_kt = (
                float(row["capacity"]) * config.CAPACITY_T_DAY_TO_KT_YR
            )
            for year in config.T_LIST[1:]:
                expected = (
                    config.AF_ENGINEERING_TSR_PATH[year]
                    * annual_capacity_kt
                    * config.TCE_PER_T_CLINKER
                )
                self.assertAlmostEqual(
                    equalized["plant_af_access_ktce"][(int(plant_id), year)],
                    expected,
                    places=9,
                )
        ...
        for province, members in plants.groupby("province"):
            for year in config.T_LIST[1:]:
                plant_ceiling = sum(
                    equalized["plant_af_access_ktce"][(int(plant_id), year)]
                    for plant_id in members.index
                )
                province_pool = equalized["province_af_pool_ktce"][(province, year)]
                self.assertGreater(plant_ceiling, province_pool)
